...
describe Bookmark do
  ...
  it "should require a url" do
    mark = Bookmark.new(@attr.merge(:url => ""))
    mark.should_not be_valid
  end
  
  it "should reject urls that are to short" do
    mark = Bookmark.new(@attr.merge(:url => "a" * 9))
    mark.should_not be_valid
  end
  ...
  it "should accept valid urls" do
    ...
    urls.each do |url|
      mark = Bookmark.new(@attr.merge(:url => url))
  ...
  it "should require a name" do
    mark = Bookmark.new(@attr.merge(:name => ""))
    mark.should_not be_valid
  end
  
  it "should reject names that are to long" do
    mark = Bookmark.new(@attr.merge(:name => "a" * 101))
    mark.should_not be_valid
  end
